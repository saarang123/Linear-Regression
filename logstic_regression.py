...
# Reshape the training and test examples (to a number between 0 and 1)
...
def propagate(w, b, X, Y):
    ...
    #Backward propagation (Grads)
    dw = (1 / m) * X.dot((A - Y).T)
    db = (1 / m) * np.sum(A - Y)
    #print("hi",A.shape, X.shape, Y.shape, dw.shape, w.shape)
    
    assert(dw.shape == w.shape)
    assert(db.dtype == float)
    cost = np.squeeze(cost) #this converts the cost to a number (from an array)
    assert(cost.shape == ())
